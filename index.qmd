...
df_hist <- df_with_dates %>%
  ...
  group_by(Year) %>%
  mutate(
    change_from_year_start = (`IXIC.Adjusted` / first(`IXIC.Adjusted`)) - 1
  ) %>%
  ungroup()
...
p <- add_trace(p, 
               ...
               text = ~paste(
                 "Year:", Year, 
                 "<br>Week:", Week, 
                 "<br>Change:", percent(change_from_year_start, accuracy = 0.1)
               ),
               showlegend = FALSE
)
...
p <- add_trace(p, 
               ...
               text = ~paste(
                 "Year: Average",
                 "<br>Week:", Week, 
                 "<br>Change:", percent(avg_change, accuracy = 0.1)
               )
)
...
if(nrow(df_current_year) > 0) {
  p <- add_trace(p, 
                 ...
                 text = ~paste(
                   "Year:", Year,
                   "<br>Week:", Week, 
                   "<br>Change:", percent(change_from_year_start, accuracy = 0.01)
                 )
  )
}
